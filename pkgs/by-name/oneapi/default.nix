...
{ lib, stdenv, fetchurl, glibc, libX11, glib, libnotify, xdg-utils, ncurses, nss, 
  ...
  pango, cairo, gdk-pixbuf, cups, expat, libxkbcommon, alsa-lib, file, at-spi2-atk,
...
stdenv.mkDerivation rec {
  ...
  srcs = [
    ...
    #  sha256 = "sha256-CUp4crvbHB4dDmXaVTSX3oGRMrKYsN32JLRQA82oKEQ=";
    #})
  ...
  libPath = lib.makeLibraryPath [ stdenv.cc.cc libX11 glib libnotify xdg-utils 
                                  ...
                                  gdk-pixbuf cups expat libxkbcommon alsa-lib
  ...
  installPhase = ''
     ...
     echo "running"
     bash $out/tmp/l_BaseKit_p_${version}_offline/install.sh --install-dir $out --download-cache $out/tmp --download-dir $out/tmp --log-dir $out/tmp --eula accept -s --silent --cli || true
